...
> **Quando usar:** Use para tarefas de performance e otimização: análise de bundle size Next.js, otimização de queries Prisma (N+1, índices), estratégias de cache (Redis), lazy loading, code splitting, Core Web Vitals, tempo de resposta de API, profiling de endpoints lentos, e otimização do dashboard em tempo real. Acione quando houver problemas de lentidão, alto uso de memória, ou para revisar código com potencial de performance crítica.
